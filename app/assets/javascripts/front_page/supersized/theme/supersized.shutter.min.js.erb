...
(function (a) {
    theme = {
        _init: function () {
            ...
            if (api.options.autoplay) {
                ...
                if (a(vars.play_button).attr("src")) {
                    a(vars.play_button).attr("src", '<%= FRONTPAGE_IMAGE_URL[:play]%>')
                }
                if (api.options.progress_bar) {
            ...
            a(vars.thumb_tray).css({
                left: -a(vars.thumb_list).width() - $(window).width() * 0.02
            });
            a(vars.tray_button).toggle(function () {
                if ($('.thumb-nav div').is(':visible')) {
                    a(vars.thumb_list).stop().animate({
                        left: $('.thumb-nav').width()
                    })
                }
            ...
            if (api.options.thumb_links) {
                ...
                a(vars.thumb_forward).click(function () {
                    if (vars.thumb_page - vars.thumb_interval <= -a(vars.thumb_list).width()) {
                        vars.thumb_page = 0;
                        a(vars.thumb_list).stop().animate({
                            left: vars.thumb_page + $('.thumb-nav').width()
                        }, {
                            duration: 500,
                            easing: "easeOutExpo"
                        })
        ...
        playToggle: function (b) {
            if (b == "play") {
                if (a(vars.play_button).attr("src")) {
                    a(vars.play_button).attr("src", '<%= FRONTPAGE_IMAGE_URL[:pause]%>')
                }
                if (api.options.progress_bar && !vars.is_paused) {
                    theme.progressBar()
                }
